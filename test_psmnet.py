...
Feature: Test PSMNet on messytable dataset
"""
...
parser = argparse.ArgumentParser(description='Testing for PSMNet')
...
parser.add_argument('--output', type=str, default='../testing_output_psmnet', help='Path to output folder')
...
# python test_psmnet.py --model /code/models/model_4.pth --onreal --exclude-bg
# python test_psmnet.py --config-file configs/remote_test.yaml --model ../train_8_14_cascade/train1/models/model_best.pth --onreal --exclude-bg --exclude-zeros --debug
...
def main():
    ...
    os.mkdir(log_dir)
    logger = setup_logger("PSMNet Testing", distributed_rank=0, save_dir=log_dir)
