...
class PSMNet(nn.Module):
    ...
    def forward(self, img_L, img_R, img_L_transformed, img_R_transformed):
        if self.training:
            pred1_l, pred2_l, pred3_l = self.subforward(img_L, img_R, img_L_transformed, img_R_transformed)
            pred1_r, pred2_r, pred3_r = self.subforward(
                img_R.flip(-1), img_L.flip(-1), img_R_transformed.flip(-1), img_L_transformed.flip(-1)
            ) # flip in W direction
            pred3_r = pred3_r.flip(-1)
            return pred1_l, pred2_l, pred3_l, pred1_r, pred2_r, pred3_r
        else:
            pred3_l = self.subforward(img_L, img_R, img_L_transformed, img_R_transformed)
