...
def train_sample(sample, psmnet_model, psmnet_optimizer, isTrain=True):
    ...
    # Train on PSMNet
    img_L = sample['img_L']    # [bs, 1, H, W]
    xL = torch.autograd.Variable(img_L.data, requires_grad=True).to(cuda_device)
    img_R = sample['img_R']    # [bs, 1, H, W]
    xR = torch.autograd.Variable(img_R.data, requires_grad=True).to(cuda_device)
    ...
    if isTrain:
        pred_disp1, pred_disp2, pred_disp3 = psmnet_model(xL, xR)
        ...
        scalar_outputs_psmnet = {'loss': 0.01*loss.item(),
