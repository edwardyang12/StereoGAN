...
class MessytableDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_L_rgb = (np.array(Image.open(self.img_L[idx].convert('RGB'))) - 127.5) / 127.5   # [H, W, 1], in (-1, 1)
        img_R_rgb = (np.array(Image.open(self.img_R[idx].convert('RGB'))) - 127.5) / 127.5   # [H, W, 1], in (-1 , 1)
